...
import { 
  ...
  Paperclip, Camera, FileImage, FileType, Eye, EyeOff
} from 'lucide-react';
...
function ClinicalMessage({ message, isUser, apiResponse = null, messageType = 'general', delay = 0 }) {
  ...
  return (
    ...
    >
      ...
      >
        {isUser ? (
          <User className="w-12 h-1 text-slate-300" />
        ) : (
         <></>
        )}
      </motion.div>
        ...
        {actions.map((action, index) => (
          <motion.button
            ...
            whileHover={{ 
              scale: 1.05, 
              y: -5,
              boxShadow: `0 20px 40px rgba(${action.color === 'blue' ? '59, 130, 246' : action.color === 'orange' ? '251, 146, 60' : action.color === 'amber' ? '245, 158, 11' : '244, 63, 94'}, 0.3)`
            }}
            whileTap={{ scale: 0.95 }}
            onClick={() => onActionSelect(action.id)}
            className={`group relative p-5 rounded-xl bg-gradient-to-br from-slate-800/60 to-slate-800/80 border border-${action.color}-500/30 backdrop-blur-md hover:border-${action.color}-400/50 transition-all duration-500 shadow-lg overflow-hidden`}
          >
            {/* Animated background glow */}
            <motion.div
              className={`absolute inset-0 bg-gradient-to-br from-${action.color}-500/10 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500`}
              animate={{
                background: [
                  `linear-gradient(135deg, rgba(${action.color === 'blue' ? '59, 130, 246' : action.color === 'orange' ? '251, 146, 60' : action.color === 'amber' ? '245, 158, 11' : '244, 63, 94'}, 0.1), transparent)`,
                  `linear-gradient(135deg, transparent, rgba(${action.color === 'blue' ? '59, 130, 246' : action.color === 'orange' ? '251, 146, 60' : action.color === 'amber' ? '245, 158, 11' : '244, 63, 94'}, 0.1))`,
                ]
              }}
              transition={{ duration: 3, repeat: Infinity }}
            />
            ...
            <div className="relative z-10">
              <motion.div
                ...
                className={`w-12 h-12 rounded-full bg-gradient-to-br from-${action.color}-400 to-${action.color}-600 flex items-center justify-center mx-auto mb-3 shadow-lg`}
              >
...
function PatientInfo({ patientInfo, setPatientInfo, isExpanded, setIsExpanded }) {
  return (
    <motion.div
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      <motion.button
        ...
        className="w-full flex items-center justify-between p-4 rounded-xl bg-gradient-to-r from-slate-800/60 to-slate-800/80 border border-slate-700/60 hover:border-slate-600/60 transition-all duration-300 mb-4"
        whileHover={{ scale: 1.02 }}
        whileTap={{ scale: 0.98 }}
      >
        <div className="flex items-center gap-3">
          ...
          <div className="text-left">
            ...
            <p className="text-xs text-gray-400">
              {patientInfo.age || patientInfo.gender || patientInfo.conditions.length > 0 
                ? 'Configuration set' 
                : 'Click to configure'
              }
            </p>
        ...
        {isExpanded && (
          ...
          >
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              >
                ...
                <input
                  ...
                  className="w-full px-4 py-3 bg-slate-900/60 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500/60 focus:ring-2 focus:ring-blue-500/20 transition-all duration-300"
                />
              </motion.div>
            ...
            >
              ...
              <textarea
                placeholder="Enter any relevant medical conditions, allergies, or medications (optional)"
                value={patientInfo.conditions.join(', ')}
                onChange={(e) => setPatientInfo({
                  ...patientInfo, 
                  conditions: e.target.value.split(',').map(s => s.trim()).filter(s => s)
                })}
                rows={3}
                className="w-full px-4 py-3 bg-slate-900/60 border border-slate-700 rounded-xl text-white text-sm focus:outline-none focus:border-blue-500/60 focus:ring-2 focus:ring-blue-500/20 transition-all duration-300 resize-none"
              />
            </motion.div>
          </motion.div>
        )}
...
function ClinicalCDSSInterface() {
  ...
  const callMedicalAPI = async (query, files = []) => {
    try {
      // If files are included, we'd need to modify this to handle file uploads
      // For now, we'll include file information in the query
  ...
  const startNewChat = () => {
    ...
    setApiError(null);
    setUploadedFiles([]);
    setPatientInfo({ age: '', gender: '', conditions: [] });
  };
  ...
  const handleQuickAction = (actionType) => {
    ...
    const prompts = {
       diet: "I need help creating a personalized nutrition plan. Please analyze dietary requirements based on the patient's medical history and current health status.",
      diagnosis: "I need diagnostic support to evaluate symptoms and medical findings. Please help analyze the clinical presentation for potential conditions.",
      prescription: "I need assistance with medication review and analysis. Please check for drug interactions, dosing, and safety considerations.",
      alert: "I need a comprehensive safety assessment for this clinical scenario. Please evaluate potential risks and provide safety recommendations."
    };
      ...
      <div className={`relative z-10 flex flex-col min-h-screen transition-all duration-300 ${showHistory ? 'ml-80' : 'ml-0'}`}>
        ...
        <div className="p-6 bg-gradient-to-r from-slate-900/95 to-slate-900/90 backdrop-blur-xl border-b border-slate-700/60 flex items-center justify-between shadow-xl">
          <div className="flex items-center gap-4">
            ...
            <div className="flex items-center gap-3">
              <Image src='/logo_1.png' width={60} height={60} alt="Logo" className="rounded-lg" />
              <div>
                <h1 className="text-xl font-bold text-white">Clinical AI Assistant</h1>
                <p className="text-xs text-gray-400">Advanced Medical Decision Support</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3">
            <motion.button
              ...
              whileTap={{ scale: 0.95 }}
              onClick={startNewChat}
              className="flex items-center gap-2 px-4 py-2 rounded-xl bg-gradient-to-r from-emerald-500 to-blue-500 text-white font-medium hover:shadow-lg hover:shadow-emerald-500/30 transition-all duration-300"
            >
            ...
            <motion.button
              whileHover={{ scale: 1.1, rotate: 90 }}
              whileTap={{ scale: 0.9 }}
              className="p-3 rounded-xl bg-slate-800/60 text-gray-400 hover:bg-slate-700/60 hover:text-white transition-all duration-300 border border-slate-700/60"
              title="Settings"
            >
        ...
        <div className="p-6 bg-gradient-to-r from-slate-900/95 to-slate-900/90 backdrop-blur-xl border-t border-slate-700/60 shadow-xl">
          {messages.length === 1 && !isTyping && ( 
            <>
              <PatientInfo 
                ...
                setIsExpanded={setPatientInfoExpanded}
              />
          ...
          <div className="relative">
            ...
            {!input && uploadedFiles.length === 0 && (
              ...
              >
                <span>💡 Try: "Analyze these symptoms" or "Check drug interactions"</span>
              </motion.div>
            )}
          </div>
